...
public class FXMLLoginDLH implements Initializable{

    @FXML
    private Button logindlh;
    ...
    private void dipencetlogin (ActionEvent event) throws IOException{
         for (int i = 0; i < listakunbaru.size(); i++) {
            ...
            if (TfNama.getText().equals(datauser.getUsername()) && TfPass.getText().equals(datauser.getPassword())) {
                ...
                Parent scene2 = FXMLLoader.load(getClass().getResource("/FXMLView/MainDLH.fxml"));
                Scene scene = new Scene(scene2);
                Stage stage = new Stage();
                stage.setScene(scene);
                stage.setTitle("Dashboard");
                stage.show();
                Stage Keluar = (Stage) logindlh.getScene().getWindow();
                Keluar.close();
